...
Outputs will be generated in a directory structure like `./<species_id>/<run_id>/` where `<species_id>` is derived from the input FASTA filename (the part before the extension) and `<run_id>` is either a timestamp or provided via `--run-id`.
...
- **Input File**: `-i, --input-files <FILES>` is required.
- **Species ID for Output**: Derived automatically from the first input filename (e.g., `my_genome.fasta` -> `my_genome`).
- **Output Directory**: Automatically created as `./<derived_species_id>/` in the current working directory. Each run creates a subdirectory: `./<derived_species_id>/<run_id>/`.
- **Run ID**: Specified with `--run-id <ID>` or defaults to a timestamp.
- **Default Outputs**: If specific paths are not provided for the following, they will be generated by default in the run-specific directory:
    - `grammar.json` (via `--output-json` or default)
    - `rules.fasta` (via `--export-blocks` or default)
    - `repeats_summary.tsv` (via `--output-repeats` or default)
    - `grammar.dot` (via `--visualize` or default)
    - `grammar.graphml` (via `--output-graphml` or default)
...
| `-j, --output-json <FILE>` | Output JSON file for the grammar.                                                                          | Defaults to `grammar.json` in the run-specific dir.    |
| `--output-text <FILE>`     | Output human-readable text representation.                                                               | Not generated by default if path not specified.        |
| `--output-gfa <FILE>`      | Output GFA format.                                                                                         | Not generated by default if path not specified.        |
| `--visualize <FILE>`       | Generate a DOT file for visualization.                                                                     | Defaults to `grammar.dot` in the run-specific dir.     |
| `--export-blocks <FILE>`   | Export grammar rules as sequences in FASTA format.                                                         | Defaults to `rules.fasta` in the run-specific dir.     |
| `--output-repeats <FILE>`  | Output tabular summary of repeats (TSV format).                                                            | Defaults to `repeats_summary.tsv` in run-specific dir. |
| `--output-graphml <FILE>`  | Output GraphML representation of the grammar.                                                              | Defaults to `grammar.graphml` in run-specific dir.   |
...
./orbweaver -i my_genome.fasta --stats
```

### Basic Usage (Multiple Files)
```bash
# Species ID will be derived from 'file1', output in ./file1/<timestamp>/
# All files (file1.fa, file2.fa) are processed into one grammar.
./orbweaver -i file1.fa,file2.fa --stats
```

### Specifying a Custom Run ID
```bash
# Output in ./my_genome/first_attempt/
./orbweaver -i my_genome.fasta --run-id first_attempt --stats
```

### Specifying an Output File (Overrides Default Location for that File)
```bash
# grammar.json will be in /custom/path/my_grammar.json
# Other defaults (rules.fasta, etc.) will be in ./my_genome/<timestamp>/
./orbweaver -i my_genome.fasta -j /custom/path/my_grammar.json
```

### Complete Analysis with All Outputs (Explicit Paths)
```bash
# Assuming output directory is ./my_genome_run/all_out/ and species_id is my_genome_run
# For this example, we create a specific structure, but usually you'd let defaults work.

# First, create the structure if you want specific non-default paths for everything
# mkdir -p custom_outputs/my_genome

# Species ID derived from input, output goes to ./my_genome_run/<run_id>
# If you want outputs elsewhere, you must specify each path.
./orbweaver -i my_genome_run.fasta --run-id all_out \
  -j custom_outputs/my_genome/grammar.json \
  --output-text custom_outputs/my_genome/grammar.txt \
  --output-gfa custom_outputs/my_genome/grammar.gfa \
  --visualize custom_outputs/my_genome/grammar.dot \
  --export-blocks custom_outputs/my_genome/rules.fasta \
  --output_repeats custom_outputs/my_genome/repeats.tsv \
  --output-graphml custom_outputs/my_genome/grammar.graphml \
  --stats
```
...
./orbweaver -i large_genome.fasta --use-encoding true --min-rule-usage 5 --stats --streaming
```
