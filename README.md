...
# Output is saved in ./input/<timestamp>/ by default.
# Default files: grammar.json, rules.fasta, repeats_summary.tsv, grammar.dot, grammar.graphml
...
# Output in ./genome1/<timestamp>/ (or specify --run-id).
./target/release/orbweaver --input-files genome1.fna,genome2.fna --stats --no-gpu

# Generate all outputs, specifying a run ID.
# Output in ./genome/<my_specific_run>/ 
# Default files (grammar.json etc.) will be in that directory unless paths are fully specified.
./target/release/orbweaver --input-files genome.fna --run-id my_specific_run \
  --output-json output/custom_grammar.json \ # Example of overriding one default path
...
# Output in ./large_genome/<timestamp>/
...
```bash
# Species ID derived from first file in list. Output dir: ./<derived_species_id>/<run_id>/
...
The primary output directory is `./<derived_species_id>/<run_id>/`, where `<derived_species_id>` is taken from the name of the first input file (without extension), and `<run_id>` is a timestamp or a user-provided string via `--run-id`.

Many common output files are generated by default inside this run-specific directory if a full path is not given for them:
...
Individual output paths can be specified to override these defaults:
...
- `--output-graphml <FILE>`: Output GraphML representation.
...
```bash
# Species ID derived from sample.fasta, output in ./sample/<timestamp>/
# Includes default outputs like grammar.json, rules.fasta, etc.
...
```bash
# Generate DOT file (e.g., ./sample/<timestamp>/grammar.dot by default)
orbweaver --input-files sample.fasta --visualize

# If you specified a path: orbweaver --input-files sample.fasta --visualize my_vis/grammar.dot

# Convert to PNG using Graphviz (assuming default path)
dot -Tpng sample/<timestamp>/grammar.dot -o sample/<timestamp>/grammar.png
```

### Memory-Efficient Processing for Large Genomes

```bash
# Use streaming mode. Output in ./large_genome/<timestamp>/
...
```bash
# GPU acceleration is enabled by default. Output in ./genome/<timestamp>/
...
# Species ID derived from file1.fasta. Output in ./file1/<timestamp>/
