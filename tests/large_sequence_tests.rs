...
// Returns the command output, the path to the output JSON file, and the TempDir (to keep it alive).
fn run_orbweaver_on_fasta(fasta_content: &str, args: &[&str]) -> Result<(std::process::Output, PathBuf, tempfile::TempDir)> {
    let dir = tempdir()?;
    ...
    cmd.current_dir(dir.path()) // Run in temp dir to avoid conflicts
        .arg("-i")
        .arg(&input_path)
        .arg("-j")
        .arg(&output_json_file_path);
    ...
    let output = cmd.ok()?;
    Ok((output, output_json_file_path, dir))
}
...
fn test_10mb_highly_repetitive_sequence_standard_mode() -> Result<()> {
    ...
    // Use ceiling division to ensure we get at least 1MB per pattern
    let one_mb_repetitions = ((1024 * 1024) + pattern_unit.len() - 1) / pattern_unit.len();
    ...
    let (output, output_json_path, _dir) = run_orbweaver_on_fasta(&fasta_content, &["--stats"])?;
...
fn test_10mb_highly_repetitive_sequence_streaming_mode() -> Result<()> {
    ...
    let (output, output_json_path, _dir) = run_orbweaver_on_fasta(&fasta_content, &["--streaming", "--chunk-size-streaming", "1048576", "--stats"])?;
